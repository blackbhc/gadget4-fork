  ...
  while(1) /* main loop over synchronization points */
    {
      ...
      static double pos[3];  // temporary vector of coorddinates
      // coordinates array
      std::unique_ptr<double[]> coordinates(new double[Sp.NumPart * 3]);
      // velocities array
      std::unique_ptr<double[]> velocities(new double[Sp.NumPart * 3]);
      // ids array
      std::unique_ptr<int[]> ids(new int[Sp.NumPart]);
      // types array
      std::unique_ptr<int[]> types(new int[Sp.NumPart]);
      // masses array
      std::unique_ptr<double[]> masses(new double[Sp.NumPart]);
      // potentials array
      std::unique_ptr<double[]> potentials(new double[Sp.NumPart]);

      for(int i = 0; i < Sp.NumPart; i++)
        {
          ids[i]    = Sp.P[i].ID.get();           // collect particle ids
          masses[i] = Sp.P[i].getMass();          // collect masses
          Sp.intpos_to_pos(Sp.P[i].IntPos, pos);  // get the pos in floating-point numbers
          // collect coordinates
          coordinates[i * 3 + 0] = pos[0];
          coordinates[i * 3 + 1] = pos[1];
          coordinates[i * 3 + 2] = pos[2];
          // collect the velocities
          velocities[i * 3 + 0] = Sp.P[i].Vel[0];
          velocities[i * 3 + 1] = Sp.P[i].Vel[1];
          velocities[i * 3 + 2] = Sp.P[i].Vel[2];
          // collect the particle types
          types[i] = Sp.P[i].getType();
          // collect the potentials
          potentials[i] = Sp.P[i].Potential;
        }
      // API of galotfa
      OnTheFly_Analysis_Nbody(All.Time, Sp.NumPart, ids.get(), types.get(), masses.get(), potentials.get(), coordinates.get(),
                              velocities.get());
#endif
